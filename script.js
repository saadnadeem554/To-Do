...
class UI {
    ...
    static loadData(projects) {
        const data = localStorage.getItem('projects');
        if (data) {
            const parsedProjects = JSON.parse(data);
            console.log(parsedProjects);
            projects.length = 0; // Clear current projects array
            parsedProjects.forEach(projData => {
                const project = new Projects(projData.project);
                projData.todos.forEach(todoData => {
                    project.addTodos(new todo(
                        todoData.title,
